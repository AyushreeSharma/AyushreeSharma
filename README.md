## Hi there 👋<h1 align="center">Hi 👋, I'm Ayushree Sharma</h1>

<p align="center">
  <b>Aspiring Data Scientist | Machine Learning Engineer | Python Developer</b>
</p>

<p align="center">
  I love building real-world AI systems, solving data problems, and turning ideas into production-ready projects.
</p>

---

## 🚀 About Me

- 🎓 Aspiring **Data Scientist / ML Engineer**
- 🤖 Currently working on:
  - Customer Churn Prediction + Drift Detection System
  - Deepfake Video Detection (research + scalability focus)
- 📊 Strong interest in **Machine Learning, Data Science & AI systems**
- 🧠 Practicing **Data Structures & Algorithms in Python**
- 🌱 Currently learning:
  - Advanced Machine Learning
  - Model Deployment (Flask / Streamlit)
  - MLOps basics
- 🎯 Goal: Build impactful AI systems used in real-world production environments

---

## 🧠 Tech Stack

### 👩‍💻 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)

### 📊 Data Science & ML
![NumPy](https://img.shields.io/badge/NumPy-013243?style=for-the-badge&logo=numpy&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![Scikit Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557C?style=for-the-badge)

### ⚙️ Tools
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github)
![VS Code](https://img.shields.io/badge/VSCode-007ACC?style=for-the-badge&logo=visual-studio-code)
![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=for-the-badge&logo=jupyter&logoColor=white)
![Google Colab](https://img.shields.io/badge/Colab-F9AB00?style=for-the-badge&logo=googlecolab&logoColor=black)

---

## 📌 Featured Projects

### 📉 Customer Churn Prediction + Drift Detection
> ML system to predict customer churn and detect data drift over time

- End-to-end ML pipeline
- Feature engineering + model training
- Focus on real-world deployment challenges
- Built using Python, Pandas, Scikit-learn

---

### 🎥 Deepfake Video Detection System
> Computer vision project for detecting manipulated videos

- Frame extraction + feature analysis
- Designed for large datasets like FaceForensics++
- Research-oriented ML approach
- Scalable architecture design

---

### 📊 Other Work
- Exploratory Data Analysis (EDA) on real datasets
- ML classification & regression models
- DSA practice in Python

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=AyushreeSharma&show_icons=true&theme=tokyonight" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=AyushreeSharma&theme=tokyonight" />
</p>

---

## 📫 Connect With Me

<p align="center">
  <a href="https://linkedin.com/in/your-link">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:your-email@example.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

## ⚡ Final Note

> “The best way to learn AI is not just studying it — but building, breaking, and improving it.”

⭐ *If you like my work, feel free to follow my journey!*
